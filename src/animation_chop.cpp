...
// NB_MODULE is a macro that creates the PyInit function for the module
// This create a method with the signature:
// PyObject* PyInit_anim_test();
// Which will return a new reference to the module
NB_MODULE(anim_test, m) {
    m.doc() = "Test methods used with the AnimationCHOP.";
	m.def("add", [](int a, int b) { 
		std::cout << "Adding " << a << " and " << b << std::endl;
		return a + b; 
	}, "Add two numbers");
	m.def("subtract", [](int a, int b) { return a - b; }, "Subtract two numbers");
}

static PyObject* cached_nanobind_anim_test_module = NULL;

// Forward declaration for the PyInit function generated by NB_MODULE.
// NB_MODULE itself provides this, but an explicit declaration can be clearer
// depending on code organization. Given NB_MODULE is above, this might not be strictly needed
// but doesn't hurt.
// extern "C" PyObject *PyInit_anim_test();

static PyObject*
anim_test_module_getter(PyObject* self, void* /* closure */) {
    // Return cached module if we already created it
    if (cached_nanobind_anim_test_module) {
        Py_INCREF(cached_nanobind_anim_test_module);
        return cached_nanobind_anim_test_module;
    }
    
    // Otherwise, initialize the nanobind module by calling its PyInit function.
    // PyInit_anim_test() returns a new reference to the module.
    cached_nanobind_anim_test_module = PyInit_anim_test();
    if (!cached_nanobind_anim_test_module) {
        // PyInit_anim_test() or nanobind internals should have set a Python error.
        return NULL;
    }
    
    // The cache now owns the reference from PyInit_anim_test().
    // The getter must return a new reference, so we Py_INCREF it.
    Py_INCREF(cached_nanobind_anim_test_module);
    return cached_nanobind_anim_test_module;
}
...
PyObject* pyGetAddFuncObject(PyObject* self, void*) {
    // Return the add function from the anim_test module
    PyObject* module = anim_test_module_getter(self, nullptr);
    if (!module) {
        return nullptr;
    }
    return PyObject_GetAttrString(module, "add");
}
...
{
	{"add", pyGetAddFuncObject, nullptr, "Get the add function from the anim_test module.", nullptr},
	{"anim_test", anim_test_module_getter, nullptr, "Module containing test methods.", nullptr},
