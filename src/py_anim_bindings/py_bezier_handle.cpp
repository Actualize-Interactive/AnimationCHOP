...
static PyObject* PyBezierHandle_new(PyTypeObject *type, [[maybe_unused]] PyObject *args, [[maybe_unused]] PyObject *kwds) {
    ...
    if (self != NULL) {
        ...
        new (&self->handle) anim::BezierHandle(0.0, 0.0);
    }
    return (PyObject *)self;
}

static void PyBezierHandle_dealloc(PyBezierHandle *self) {
    self->handle.~BezierHandle();
...
static int PyBezierHandle_set_value(PyBezierHandle *self, PyObject *value, [[maybe_unused]]void *closure) {
    ...
    self->handle.value = PyFloat_AsDouble(value);
    return 0;
}


PyGetSetDef PyBezierHandle_getset[] = {
    {"time", (getter)PyBezierHandle_get_time, (setter)PyBezierHandle_set_time, "Time of the handle point", NULL},
    {"value", (getter)PyBezierHandle_get_value, (setter)PyBezierHandle_set_value, "Value of the handle point", NULL},
    {NULL}  // Sentinel
};

static PyObject* PyBezierHandle_str(PyBezierHandle *self) {
    auto str = std::format("BezierHandle(time={}, value={})", self->handle.time, self->handle.value);
...
[[maybe_unused]] PyBezierHandle* BezierHandleToPyBezierHandle(const anim::BezierHandle& handle) {
    ...
    new (&pyHandle->handle) anim::BezierHandle(handle);
    return pyHandle;
}
...
[[maybe_unused]] bool PyObjectToBezierHandle(PyObject* obj, anim::BezierHandle& handle) {
    if (PyObject_TypeCheck(obj, &PyBezierHandleType)) {
        ...
        handle = pyHandle->handle;
        return true;
    }
    
    // Try to get time and value attributes (for backward compatibility with Point2D)
    PyObject* time_attr = PyObject_GetAttrString(obj, "time");
    PyObject* value_attr = PyObject_GetAttrString(obj, "value");
    
    if (time_attr && value_attr && PyFloat_Check(time_attr) && PyFloat_Check(value_attr)) {
        handle.time = PyFloat_AsDouble(time_attr);
        handle.value = PyFloat_AsDouble(value_attr);
        Py_DECREF(time_attr);
        Py_DECREF(value_attr);
        return true;
    }
    
    Py_XDECREF(time_attr);
