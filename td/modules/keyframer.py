...
class Channel(Keyframer):
	def __init__(self, owner, name, _id, index, startVal=None):
		...
		if self.keysComp.op(self.name):
			...
			self.keysDat = self.keysComp.copy(self.keysTemplateDat, 
													name=self.name)
			self.segments = []
			self.insertKey(self.AnimationChop.par.end.eval(), startVal, 
	...
	def insertKey(self, x, y=None, funcName=None):
		...
		if funcName is not None:
			self.AnimationChop.setKeyframe(x, channel=self.name, 
										value=y, function=funcName)
		else:
			self.AnimationChop.setKeyframe(x, channel=self.name, value=y)			
	...
	def deleteKey(self, i):
		...
		for col in metadata:
			col.pop(n + 1)
		if i == self.numSegments:
			self.AnimationChop.deleteKeyframe(self.segments[i-1].x1,
															channel=self.name)
		else:													
			self.AnimationChop.deleteKeyframe(self.segments[i].x0,
															channel=self.name)
		for j in range(3):
